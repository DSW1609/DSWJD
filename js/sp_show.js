...
// 分类栏点击弹出隐藏栏效果
// 获取到所有的可点击的s-div
var as_s = document.querySelectorAll("s");
// 获取到对应的可展开的行
var as_hang = document.querySelectorAll(".as_hang");
// 循环添加自定义属性data-flag判断当前的状态是否展开
for (let i = 0; i < as_s.length; i++) {
  as_s[i].setAttribute("data-flag", "true");
}
// 循环注册点击事件
// 最后一个没有点击事件所以减一
for (let i = 0; i < as_s.length - 1; i++) {
  as_s[i].addEventListener("click", function () {
    // 获取到自定义属性
    var data_as = this.getAttribute("data-as");
    if (this.getAttribute("data-flag") == "true") {
      // 根据自定义属性展开对应的隐藏行
      as_hang[data_as].style.height = "auto";
      // 点击谁给谁添加class类名用来更换s标签的背景图标
      as_hang[data_as].classList.add("as_jian");
      this.setAttribute("data-flag", "false");
    } else {
      // 根据自定义属性展开对应的隐藏行
      as_hang[data_as].style.height = "30px";
      // 点击谁给谁添加class类名用来更换s标签的背景图标
      as_hang[data_as].classList.remove("as_jian");
      this.setAttribute("data-flag", "true");
    }
  })
}
// 店铺热销关注tab栏切换事件
var as_hot_tit = document.querySelectorAll(".as_hot_tit");
var as_hot_list = document.querySelectorAll(".as_hot_list");
// 循环注册点击事件
for (let i = 0; i < as_hot_tit.length; i++) {
  as_hot_tit[i].addEventListener("click", function () {
    // 首先清清除所有的点击的class样式并隐藏下方的内容
    for (let i = 0; i < as_hot_tit.length; i++) {
      as_hot_tit[i].classList.remove("as_hot_red");
      as_hot_list[i].style.display = "none";
    }
    // 点击谁给谁添加class样式
    this.classList.add("as_hot_red");
    // 获取自定义属性
    var data_ashot = this.getAttribute("data-ashot");
    // 根据自定义属性显示对应的下方内容
